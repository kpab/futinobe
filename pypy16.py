...
class Agent():
    ...
    def move(self, other_agents, maze): # pathリスト順に位置を更新
        if len(self.path)>self.speed-1: # ゴールまで1フレームのspeedで行けるか否か
            ...
            # neighborsを取得
            neighbors = ag.getNeighbors(next_position, other_agents, maze, object_cost)
            # --- 衝突判定 ---
            avoid = False
            ...
            if not avoid:
                ...
                if not neighbors:
                    ...
                    self.position = neighbors[random.randint(0, len(neighbors)-1)]
        else: # ゴール
...
def simulation(SIMU_COUNT):
    ...
    def update(frame):
        ...
        ag.nowAgentImpactChk(agents, maze)
        ag.wallBreakerChk(agents, wall_list, maze, object_cost)
        # --- 新規エージェント ---

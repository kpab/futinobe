...
def simulation(SIMU_COUNT):
    ...
    def update(frame):
        ...
        for agent in agents:
            ...
            if not agent.path:  # パスない場合次のエージェントへ
                continue
            (x, y) = agent.position
            back_posi = (x - (agent.path[0][0]-agent.position[0]),
                         y - (agent.path[0][1]-agent.position[1]))
            neighbors = [(x - 1, y), (x + 1, y), (x, y - 1), (x, y + 1)]
            ...
            for agent_2 in agents:
                ...
                if agent.id != agent_2.id and agent.path[0] == agent_2.path[0]:
                    ...
                    neighbors = [pos for pos in neighbors if pos !=
                                 agent.path[0]]  # 予定パスを除く
                    avoid = True
            if not avoid:
                continue
            # --- 移動範囲の制限 ---
            can_neighbors = [
                ...
                # 他のエージェントがいない場所をピック
                not any(pos == a.position for a in agents) and
    ...
    ani = animation.FuncAnimation(
        fig, update, frames=SIMU_COUNT, interval=600, repeat=False)
    plt.show()

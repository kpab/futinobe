...
import heapq
import datetime
import modules.Agent_Data as ag_data
...
#     # --- 移動 ---
#     def move(self, other_agents, maze):  # pathリスト順に位置を更新
#         if len(self.path) > self.speed-1:  # ゴールまで1フレームのspeedで行けるか否か
#             for _ in range(self.speed):  # speed分移動
#                 next_position = self.path.pop(0)
#             # neighborsを取得
#             neighbors = ag.getNeighbors(
#                 next_position, other_agents, maze, object_cost)
#             # --- 衝突判定 ---
#             avoid = False
...
#                 print(f"{self.id}は予定通り")
#             else:
#                 if not neighbors:
#                     print(f"{self.id}はどこにも行けない…")
#                     return
#                 else:
#                     print(f"{self.id}は回避した")
#                     self.position = neighbors[random.randint(
#                         0, len(neighbors)-1)]
#         else:  # ゴール
...
def simulation(SIMU_COUNT):
    ...
    # --- マップ生成 ---
    map = Map.Map(object_cost, MAP_SIZE_X, MAP_SIZE_Y,
                  start_list, wall_list, goal_list)
    ...
    def update(frame):
        ...
        for agent in agents:
            ...
            if len(agent.path) < 1:
                agents.remove(agent)
            agent.move([a for a in agents if a != agent], maze)
            ax.scatter(agent.position[1], agent.position[0], c=agent.color)
            ax.text(agent.position[1], agent.position[0], agent.id)
            ax.set_title(f"シミュレーション: {s}")
            result.append(agent.info())

        ag_data.nowAgentImpactChk(agents, maze)
        ag_data.wallBreakerChk(agents, wall_list, maze, object_cost)
        # --- 新規エージェント ---

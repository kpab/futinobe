...
def simulation(SIMU_COUNT):
    ...
    fig, ax = plt.subplots(figsize=(MAP_SIZE_X, MAP_SIZE_Y),
                           facecolor=color_list[random.randint(0, len(color_list)-1)])
    ...
    def update(frame):
        ...
        for agent in agents:
            (x, y) = agent.position
            neighbors = [(x - 1, y), (x + 1, y), (x, y - 1), (x, y + 1)]
            # can_neighbors = neighbors[:]
            for agent_2 in agents:
                if not agent.path or not agent_2.path:
                    continue
                # 異なるidで次の場所が同じ
                if agent.id != agent_2.id and agent.path[0] == agent_2.path[0]:
                    print(
                        f"シミュレーション{s}の{agent.id}と{agent_2.id}が{(x, y)}で衝突の危機")
                    # # --- 移動範囲の制限 ---
                    # for next_posi in neighbors:
                    #     if (next_posi[0] < 0 or next_posi[0] >= len(maze) or
                    #         next_posi[1] < 0 or next_posi[1] >= len(maze[0]) or
                    #         maze[next_posi[0]][next_posi[1]] == object_cost):
                    #         if next_posi in can_neighbors:
                    #             can_neighbors.remove(next_posi)
                    #     elif next_posi == agent.path[0]: # pathの最初除去
                    #         if next_posi in can_neighbors:
                    #             can_neighbors.remove(next_posi)
                    for next_posi in neighbors:
                        if (next_posi[0] < 0 or next_posi[0] >= len(maze) or
                    ...
                    continue
            # if not can_neighbors: # 避けれない場合停止
            #     print(f"{agent.id}停止!")
            ...
            if not neighbors:  # 避けれない場合停止
                print(f"{agent.id}停止!")
                agent.path.insert(0, agent.position)
            else:
                agent.position = neighbors[random.randint(
                    0, len(neighbors)-1)]  # neighborsからランダムに移動
                agent.path = agent.calc_path(maze)

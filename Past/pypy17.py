...
class Boids():
    def __init__(self, agents):
        ...
        for agent in agents:
            for i in range(len(goal_list)):
                if agent.r == i:
                    self.agents += [{'p': agent.position,
                                     'v': agent.speed, 'r': agent.goal}]
    ...
    def distance(self, agents):
        for i in range(len(agents)):
            for j in range(len(agents)):
                d = np.array(self.agents[i]['p']) - \
                    np.array(self.agents[j]['p'])
    ...
    def ruleSeparate(self, n):
        a = np.array(np.where((self.dist[n] < RANGE_Separate) & (
            self.dist[n] > 0)), dtype=int)[0]
        v = np.zeros(2)
        cnt = 0
        for i in a:
            if i != n:
                d = np.array(self.agents[n]['p']) - \
                    np.array(self.agents[i]['p'])
                if self.dist[n][i] != 0:
                    v += d / self.dist[n][i]**2
                    cnt += 1
        if cnt == 0:
    ...
    def ruleAlignment(self, n):
        a = np.array(np.where((self.dist[n] < RANGE_Alignment) & (
            self.dist[n] > 0)), dtype=int)[0]
        v = np.zeros(2)
        cnt = 0
        for i in a:
    ...
    def ruleMokuteki(self, n, agents):
        ...
        for i in range(len(agents)):
            ...
            # 距離がゼロでない場合のみ計算
            if np.linalg.norm(np.array(self.agents[n]['p']) - np.array(self.agents[n]['r'])) > 0:
                direction = np.array(
                    self.agents[n]['r']) - np.array(self.agents[n]['p'])
    ...
    def simulation(self, agents):
        ...
        vel_tmp = []
        for i in range(len(agents)):
            vel = [self.ruleSeparate(i)*0.5 + self.ruleAlignment(
                i)*0.3 + self.ruleCohesion(i, agents)*0.2 + self.ruleMokuteki(i, agents)*0.3]
